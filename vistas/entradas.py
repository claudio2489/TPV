...
from vistas import entradasCarga
...
class VistaEntradas(QtWidgets.QWidget):
	#Inicializacion del Objeto QWidget
	def __init__(self):
		QWidget.__init__(self)

		#Importamos la vista "listaAfiliados" y la alojamos dentro de la variable "vistaLista"
		self.vista_entradas = uic.loadUi("gui/entradas/entradas.ui", self)
		self.vista_entradas.btn_agregarentrada.clicked.connect(self.AbrirEntradasCarga)


	def AbrirEntradasCarga(QWidget):
		#variables que alojan las clases que se encuentran dentro del archivo .py. (nombredelArchivo.nombredelaClase)
		widget_entradascarga = entradasCarga.VistaEntradasCarga()
		#hacemos correr la ventana
		widget_entradascarga.show()