...
from vistas import platosCarga
...
class VistaPlatos(QtWidgets.QWidget):
	#Inicializacion del Objeto QWidget
	def __init__(self):
		QWidget.__init__(self)

		#Importamos la vista "listaAfiliados" y la alojamos dentro de la variable "vistaLista"
		self.vista_platos = uic.loadUi("gui/platos/platos.ui", self)
		self.vista_platos.btn_agregarplato.clicked.connect(self.AbrirPlatosCarga)


	def AbrirPlatosCarga(QWidget):
		#variables que alojan las clases que se encuentran dentro del archivo .py. (nombredelArchivo.nombredelaClase)
		widget_platos = platosCarga.VistaPlatosCarga()
